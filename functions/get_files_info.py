...
def get_files_info(working_directory: str, directory: str = ".") -> str:
    print(f"Result of '{directory}' directory:")
    try:
        ...
        list_file_specs_in_directory = []
        for file in list_directory:
            file_path = os.path.join(target_dir, file)
            list_file_specs_in_directory.append(
                f"- {file}: file_size={os.path.getsize(file_path)} bytes, is_dir={os.path.isdir(file_path)}")

        list_file_specs_in_directory_string = "\n".join(
            list_file_specs_in_directory)

        return list_file_specs_in_directory_string
